%% Description errorb_old(x,y,varargin)
% errorb_old(Y,E) plots Y and draws an error bar at each element of Y. The
...
% bars.
%
% errorb_old(X,Y,E) plots Y versus X with
...
%
% errorb_old(X,Y,'Parameter','Value',...) see below
%
%% Optional Parameters
...
% errorb_old(values,errors);
% errorb_old(values,errors,'top');
%
% % motivation for the function
% It is possible to plot nice error bars on top of a bar plot with Matlab's
% built in errorb_oldar function by setting tons of different parameters to be
% various things.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% ADAPTED for Matlab 2017b
% Leandro Bezerra Marinho, last update Sep 2018,
%               leandrobezerramarinho@gmail.com
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [graph] = errorb(x,y,varargin)

graph = errorb_old(x,y,varargin{:});

inputExist = find(cellfun(@(x) strcmpi(x, 'BarColor') , varargin));
if inputExist
    bar_color = varargin{inputExist+1};
    
    for i = 1 : length(graph)
        graph(i).FaceColor = bar_color(i, :);
    end
end
...
function [graph] = errorb_old(x,y,varargin)
...
if size(x,1)>1 && size(x,2)>1 % if you need to do a group plot
    ...
    for i = 1:num2Plot
        ...
        errorb_old(x,y(:,i), e(:,i),'barwidth',1/(num2Plot),varargin{:}); %errorb_old(x,y(:,i), e(:,i),'barwidth',1/(4*num2Plot),varargin{:});
    end
    if ~hold_state
        hold off;
    end
